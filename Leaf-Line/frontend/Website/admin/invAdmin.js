...
// Updated Retrieve Investor ID functionality
document.getElementById("retrieve-id-btn")?.addEventListener("click", async () => {
    const email = document.getElementById("retrieve-id-email").value;
    const password = document.getElementById("retrieve-id-password").value;
    const resultEl = document.getElementById("investor-id-result");
    ...
    try {
        // Show loading state
        resultEl.textContent = "Authenticating with Firebase...";
        ...
        // 2. Call your backend API to get investorID
        const response = await fetch('/api/investor/id', {  // Changed from '/api/investors/id' to '/api/investor/id'
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Failed to fetch investor ID');
        }
        ...
        if (data.investorID) {
            resultEl.textContent = `Your Investor ID: ${data.investorID}`;
            resultEl.className = "result-container success";
        } else {
            throw new Error("Investor ID not found in response");
        }
